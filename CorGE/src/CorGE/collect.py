...
def check_assembly_summary():
    if not os.path.exists(os.path.join(OUTPUT_FP, "assembly_summary.txt")):
        ...
        with open(
            os.path.join(OUTPUT_FP, "assembly_summary.txt"), "a"
...
def fetch_genome(acc: str or None):
    if acc:
        ...
        if f"{acc}.fna" in os.listdir(NUCL_OUTPUT_FP):
            ...
            download(
                url_prefix, full_acc, acc, NUCL_OUTPUT_FP, "_cds_from_genomic", ".fna"
            )
    ...
    for n in nucl_list:
        if n.replace(".fna", ".faa") in prot_list:
            if dryrun:
                ...
                shutil.copyfile(
                    os.path.join(dir, n.replace(".fna", ".faa")),
                    os.path.join(PROT_OUTPUT_FP, n.replace(".fna", ".faa")),
                )
...
def collect_outgroup(id: str, dryrun: bool):
    if dryrun:
        ...
        try:
            os.rename(
                os.path.join(NUCL_OUTPUT_FP, f"{id}.fna"),
                os.path.join(OUTGROUP_OUTPUT_FP, f"{id}.fna"),
            )
            os.rename(
                os.path.join(PROT_OUTPUT_FP, f"{id}.faa"),
                os.path.join(OUTGROUP_OUTPUT_FP, f"{id}.faa"),
            )
        except OSError:
